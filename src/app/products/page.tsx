...
import { seed19ProductsToFirestore } from '@/lib/seedProducts';
...
export default function ProductsPage() {
  ...
  const handleSeedProducts = async () => {
    setIsSeeding(true);
    try {
      const { seededCount } = await seed19ProductsToFirestore();
      setSuccessMessage(`Seeded ${seededCount} authentic Aqua Point products to Firestore!`);
        ...
        <div className="glass-panel rounded-2xl p-16 text-center space-y-3">
          ...
          <div className="flex items-center justify-center gap-3 pt-2">
            <button
              onClick={handleSeedProducts}
              disabled={isSeeding}
              className="px-4 py-2 text-xs font-bold rounded-xl bg-cyan-500 text-slate-950 hover:bg-cyan-400 cursor-pointer shadow-[0_0_15px_rgba(0,229,255,0.3)] flex items-center gap-1.5 disabled:opacity-50"
            >
