...
export default function ProductsPage() {
  ...
  const handleSaveProduct = async (e: FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        // Upload directly to Cloudinary preset 'aqua_point' with folder 'products'
      ...
      if (editingProductId) {
        ...
        setSuccessMessage(`Product "${name}" added to Firestore catalog!`);
      }
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Real-time Cloud Firestore sync (<code className="text-cyan-400">products</code>) & Cloudinary media folder (<code className="text-cyan-400">products/</code>).
          </p>
        </div>
      ...
      {/* Control Bar: Search, Category Filter & View Modes */}
      ...
      {loading ? (
        <div className="glass-panel rounded-2xl p-16 flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-xs text-slate-400">Loading products from Cloud Firestore...</p>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-16 text-center space-y-3">
          ...
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            {searchTerm || selectedCategoryFilter !== 'All'
              ? 'No products match your selected search terms or filters.'
              : 'There are currently no products in your Cloud Firestore catalog.'}
          </p>
          <button
            onClick={openAddModal}
          ...
          <div className="glass-panel-cyan w-full max-w-2xl rounded-3xl p-6 relative space-y-5 my-8 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between pb-4 border-b border-white/10">
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                ...
                {editingProductId ? 'Edit Product Details' : 'Add New Product to Firestore'}
              </h2>
              <button
            ...
            <form onSubmit={handleSaveProduct} className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-900/60 border border-white/10">
                  ...
                  <label htmlFor="featuredToggle" className="text-xs text-slate-200 cursor-pointer font-medium flex items-center gap-1.5">
                    ...
                    Highlight as Featured Product (Shows on homepage hero showcase)
                  </label>
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">
                  Product Image (Cloudinary Cloud: <span className="text-cyan-400">rvoym2gw</span> | Folder: <span className="text-cyan-400">products/</span>)
                </label>
                <div className="border-2 border-dashed border-cyan-500/30 rounded-2xl p-4 text-center hover:border-cyan-400 transition-colors bg-slate-900/50">
                  {previewUrl || existingImageUrl ? (
                    ...
                    <label className="cursor-pointer flex flex-col items-center justify-center space-y-2 py-2">
                      ...
                      <span className="text-[10px] text-slate-400 font-mono">Preset: aqua_point | Target Folder: products</span>
                      <input
                        type="file"
                        accept="image/*"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="glass-panel w-full max-w-md rounded-3xl p-6 border-rose-500/30 space-y-4 animate-in fade-in zoom-in-95 duration-150">
            ...
            <p className="text-xs text-slate-300">
              Are you sure you want to permanently remove <strong className="text-white">"{deletingProduct.name}"</strong> from your Cloud Firestore products catalog?
            </p>
